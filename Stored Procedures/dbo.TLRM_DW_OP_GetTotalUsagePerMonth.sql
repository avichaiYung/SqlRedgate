...
-- =============================================
CREATE PROCEDURE [dbo].[TLRM_DW_OP_GetTotalUsagePerMonth]
	@orderProductID int,
	@InternalOrderID int,
	@FromDate datetime,
	@ToDate datetime,
...
DECLARE	
	...
	SET @sqlStr = '	SELECT TLRM_USG_Usage.usageYear, TLRM_USG_Usage.usageMonth, ISNULL(SUM(CASE WHEN usageWebbingSIM = 1 THEN 0 ELSE usageQuota END), 0) AS TotalWebbingUsage, 
	...
	SET @sqlStr = @sqlStr + ' GROUP BY TLRM_USG_Usage.usageYear, TLRM_USG_Usage.usageMonth '

	SET @sqlStr = @sqlStr + ' ORDER BY TLRM_USG_Usage.usageYear DESC, TLRM_USG_Usage.usageMonth DESC'
